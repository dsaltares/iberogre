...
void EscenaSimple::crearEscena() {
    ...
    entidadPersonaje = gestorEscena->createEntity("entidadPersonaje", "Sinbad.mesh");
...
void EscenaSimple::actualizarPersonaje(Ogre::Real deltaT) {
    ...
    if (andando) {
        nodoPersonaje->translate(Ogre::Vector3(0.02, 0.02, 0.02) * direccion * deltaT, Ogre::Node::TS_WORLD);
        
        Ogre::Quaternion rotacion = direccionPersonaje.getRotationTo(direccion, Ogre::Vector3::UNIT_Y);
