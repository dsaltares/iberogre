...
AplicacionOgre::AplicacionOgre(const Ogre::String& ventana): nombreVentana(ventana) {
    
    // Si iniciar Ogre u OIS falla, debemos salir
    salir = (!iniciarOgre() || !iniciarOIS() || !iniciarSDL());
}
...
bool AplicacionOgre::iniciarOgre() {
    ...
    // Creamos los gestores de sonido
    gestorSonido = new SoundFXManager();
    gestorMusica = new SongManager();

    // Diálogo para configurar Ogre
    if (!raiz->restoreConfig() && !raiz->showConfigDialog()) {
        log->logMessage("AplicacionOgre::iniciarOgre(): El usuario ha cancelado el diálogo de configuración");
        return false;
    }
    ...
    if (raiz->initialise(true, nombreVentana)) {
        ...
        log->logMessage("AplicacionOgre::iniciarOgre(): No se pudo inicializar Ogre");
        return false;
    }
...
bool AplicacionOgre::iniciarOIS() {
   ...
   parametrosOIS.insert(std::make_pair(string("XAutoRepeatOn"),
    ...
    teclado = static_cast<OIS::Keyboard*>(gestorEntrada->createInputObject(OIS::OISKeyboard,
                                                                           true));
    raton = static_cast<OIS::Mouse*>(gestorEntrada->createInputObject(OIS::OISMouse,
                                                                      true));
...
bool AplicacionOgre::iniciarSDL() {
...
AplicacionOgre::~AplicacionOgre() {
    ...
    // Destruimos los gestores de sonido
    delete gestorSonido;
    delete gestorMusica;
...
bool AplicacionOgre::mousePressed(const OIS::MouseEvent &arg, OIS::MouseButtonID id) {
    return true;
}

bool AplicacionOgre::mouseReleased(const OIS::MouseEvent &arg, OIS::MouseButtonID id) {
    return true;
}
